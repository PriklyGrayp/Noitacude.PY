...
def write_grayscale(filename, pixels):
    ...
    Args:
        ...
        pixles: A rectangular image stored as a sequence of rows.
        Each row must be an iterable series of integers in the range 0-255

    Raises:
        OSError: if the fil;e could not be written.
    '''
    height = len(pixels)
    width = len(pixels[0])
    ...
    with open(filename, 'wb') as bmp:
        ...
        size_bookmark = bmp.tell()  # The next fout bytes hold the filesize as a 32-bit little-endian integer. Zero placeholder for now.
        bmp.write(b'\x00\x00\x00\x00')

        bmp.write(b'\x00\x00')  # Unused 16-bit integer - should be zero
        bmp.write(b'\x00\x00')  # Unused 16-bit integer - should be zero

        pixel_offset_bookmark = bmp.tell()  # The next fout bytes hold the filesize as a 32-bit little-endian integer. Zero placeholder for now.
        bmp.write(b'\x00\x00\x00\x00')

        # Image Headder
        bmp.write(b'\x28\x00\x00\x00')  # Image headder size in bytes - 40 decimal
...
def dimentions(filename):
    '''Determine the dimentions in pixels of a bmp image.
